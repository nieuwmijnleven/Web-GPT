...
4. **OAuth gateway** — check `devspace-oauth-gateway.service`, `curl -fsS http://127.0.0.1:9292/healthz`, and the two `/.well-known/` responses. The gateway must preserve DevSpace's loopback resource and authorization-server metadata; tunnel-client and the OpenAI tunnel service handle the tunnel-facing path. A gateway 502 means the fixed DevSpace upstream is unavailable.
